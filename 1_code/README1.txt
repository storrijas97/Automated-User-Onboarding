...
- OrangeHRM 5.x (for HR sync; direct MySQL DB access required)
...
2. Edit server/.env and fill in the following values:
     ...
     ORANGE_HRM_DB_HOST=<orangehrm-server-ip>
     ORANGE_HRM_DB_USER=orange
     ORANGE_HRM_DB_PASSWORD=orange
     ORANGE_HRM_DB_NAME=orangehrm
...
- If Active Directory is not configured, the system will still run but
  AD operations will fail gracefully and log errors to the audit log.
